...
export const FocusSession: React.FC = () => {
  ...
  const openPickerFor = (act: 'mining'|'fishing') => { setActivity(act); setShowPicker(true); };
  const begin = () => { startFocus(activity, minutes); localStorage.setItem(LAST_DURATION_KEY, String(minutes)); localStorage.setItem(LAST_ACTIVITY_KEY, activity); setShowPicker(false); };
