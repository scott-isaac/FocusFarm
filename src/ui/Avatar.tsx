...
export const Avatar: React.FC = () => {
  const { focusActive } = useGameStore();
  ...
  return (
    <div className="avatar-wrapper">
      <div className={`avatar-stage ${focusActive? 'avatar-mining':'avatar-idle'}`}>        
        <div className="avatar-char">
          ...
          <div className="avatar-spark" ref={sparkRef} />
        </div>
      </div>
      <div className="avatar-label">{focusActive? 'Mining' : 'Idle'}</div>
    </div>
  );
};
